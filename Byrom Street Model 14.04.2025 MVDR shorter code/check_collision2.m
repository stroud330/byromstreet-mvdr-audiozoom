function [collision_array2, collision_coords2, calculated_normal2, no_collision_idx2] = check_collision2(start_point, end_point, world_objects, soundProfile, collision_group, surfaces, dimensions, reflection_group)
% Initialize outputs
    ...
    collision_coords2 = NaN(total_reflected_lines, 3);  
    % Initialize a vector to keep track of lines with no collisions
    no_collision_idx2 = [];
    % Initialize collision_array
    collision_array2 = repmat(struct('SoundSource', '', 'SoundProfile', '', 'Component', '', 'Material', '', 'Sabine_Coefficient', '', 'Collision_Coordinates', [], 'Normal', []), total_reflected_lines, 1);
    ...
    for i = 1:length(collision_array2)
        collision_array2(i).SoundSource = SoundSource{i};
        collision_array2(i).SoundProfile = SoundProfile{i};
    end
    ...
    for building_idx2 = 1:length(world_objects)
        for component_idx = 1:length(world_objects(building_idx2).components)
           ...
           if isempty(vertices2) || isempty(faces2)
            ...
            calculated_normal2 = cross(v2 - v1, v3 - v1);

            t = dot(calculated_normal2, (v1 - current_start_point)) / dot(calculated_normal2, (current_end_point - current_start_point));
                isIntersect2 = false;
                point2 = [];
...
isWithinBounds2 = isWithinBoundsXY2 && isWithinBoundsZ2;
        ...
        closest_collision_distance2(line_idx2) = distance_to_collision;
                            collision_array2(line_idx2).Component = this_component2.name;
                            collision_array2(line_idx2).Material = this_component2.material;
                            collision_array2(line_idx2).Sabine_Coefficient = this_component2.sabine_coefficient;
                            collision_array2(line_idx2).Collision_Coordinates = point2;
                            collision_array2(line_idx2).Normal = calculated_normal2;
                          if isCeiling
                                collision_array2(line_idx2).Component = strcat(this_component2.name, '_ceiling');
                          end
            ...
            if closest_collision_distance2(line_idx2) == Inf
                no_collision_idx2 = [no_collision_idx2, line_idx2];
            end
 ...
 %% Ground Collision Checks
    for surface_idx = 1:length(surfaces)
        ...
        if ~isempty(point2) && inpolygon(point2(1), point2(2), x, y)
            ...
            if distance_to_collision < closest_collision_distance2(line_idx2) && distance_to_collision > tolerance_for_start_point
        closest_collision_distance2(line_idx2) = distance_to_collision;
                collision_array2(line_idx2).Component = surfaces(surface_idx).name; % Assuming surfaces struct has a name field
                collision_array2(line_idx2).Material = surfaces(surface_idx).material;
                collision_array2(line_idx2).Sabine_Coefficient = surfaces(surface_idx).sabine_coefficient;
                collision_array2(line_idx2).Collision_Coordinates = point2;
                collision_array2(line_idx2).Normal = calculated_normal2;
            end
        end
...
%% Preallocate collision_coords with NaN
collision_coords2 = NaN(length(collision_array2), 3);

% Collect the coordinates from the struct
for i = 1:length(collision_array2)
    if ~isempty(collision_array2(i).Collision_Coordinates)
        collision_coords2(i, :) = collision_array2(i).Collision_Coordinates;
    end
end

% Plot the collision coordinates
if ~all(isnan(collision_coords2), 'all')
    valid_coords2 = collision_coords2(~isnan(collision_coords2(:, 1)), :);
