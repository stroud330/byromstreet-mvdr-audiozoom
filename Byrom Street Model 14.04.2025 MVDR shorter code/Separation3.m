function final_separated_signal3 = Separation3(final_separated_signal2, beamform_grid_choice, fs)
    % Ensure the audio is mono
    if size(final_separated_signal2, 2) > 1
        final_separated_signal2 = mean(final_separated_signal2, 2); % Convert to mono by averaging channels
    end

    % Ensure final_separated_signal is a column vector
    if isrow(final_separated_signal2)
        final_separated_signal2 = final_separated_signal2';
    end

    original_length = length(final_separated_signal2); % Store original length
    ...
    [S, ~, ~] = spectrogram(final_separated_signal2, window, noverlap, nfft, fs);
    magnitude = abs(S);
    phase = angle(S);
    ...
    for i = 1:num_components + 1
        filename = sprintf('Separated_Audio_Grid_%s_Source_%dc.wav', num2str(beamform_grid_choice), i);
        audiowrite(filename, separated_signals{i}, fs);
        fprintf('Saved %s\n', filename);
    end

    % Save the last separated signal as a variable
    final_separated_signal3 = separated_signals{num_components + 1};
    ...
    for i = 1:num_components + 1
        ...
        title(sprintf('Separated Source %dc', i));
        xlabel('Time (s)');
        ylabel('Amplitude');
        axis([0 5 -1 1]);
