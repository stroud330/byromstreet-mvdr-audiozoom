function [final_filtered_signal] = Filter(beamformed_audio, beamform_grid_choice, fs)
%% Spectral Mask + Wiener Filtering
    ...
    final_filtered_signal = reconstructed_signal;

    % Save the separated audio file
    filename_final = sprintf('Filtered_Speech_Grid_%s.wav', num2str(beamform_grid_choice));
    audiowrite(filename_final, final_filtered_signal, fs);
    fprintf('Saved %s\n', filename_final);
